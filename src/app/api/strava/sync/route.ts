...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log('Fetching Strava activities for user:', userId);
    const activitiesRaw: StravaActivity[] = await getStravaActivities(
      accessToken
    );

    if (!Array.isArray(activitiesRaw)) {
      console.error('Strava API returned non-array:', activitiesRaw);
      return NextResponse.json(
    ...
    console.log(`Retrieved ${activitiesRaw.length} activities from Strava`);
